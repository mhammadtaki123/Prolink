<?php
/**
 * ProLink – Worker: My Bookings
 * Path: /Prolink/worker/bookings.php
 * - Requires worker login
 * - Lists worker's bookings with filters + pagination
 * - Safe datetime formatting (NULL/zero date aware)
 * - Actions: Accept (pending), Complete (accepted), Cancel (pending/accepted)
 * - CSRF protection
 */
...
if (empty($_SESSION['worker_id'])) { header('Location: ' . $baseUrl . '/auth/worker-login.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['csrf_token']) && hash_equals($csrf, $_POST['csrf_token'])) {
  ...
  if ($bid <= 0) { $err = 'Invalid booking id.'; }
    ...
    // Ensure the booking belongs to this worker
    $st = $conn->prepare("SELECT booking_id, status FROM bookings WHERE booking_id = ? AND worker_id = ? LIMIT 1");
    if (!$st) { $err = 'Prepare failed: ' . $conn->error; }
    else {
      $st->bind_param('ii', $bid, $worker_id);
      $st->execute();
      ...
      if (!$bk) { $err = 'Booking not found.'; }
        ...
        if ($act === 'update') {
          ...
          if (!in_array($to, $allowed, true)) { $err = 'Invalid status.'; }
            ...
            $okTransitions = [
              'pending'  => ['accepted','cancelled'], // accept job or decline (cancel)
              'accepted' => ['completed','cancelled'], // complete or cancel
              'completed'=> [],
              'cancelled'=> []
            ];
            if (!in_array($to, $okTransitions[$cur] ?? [], true)) {
              ...
              $u = $conn->prepare("UPDATE bookings SET status = ? WHERE booking_id = ? AND worker_id = ?");
              if (!$u) { $err = 'Prepare failed: ' . $conn->error; }
              else {
                $u->bind_param('sii', $to, $bid, $worker_id);
...
if ($q !== '') {
  $where[] = "(s.title LIKE ? OR s.location LIKE ? OR u.full_name LIKE ?)";
  $like = '%'.$q.'%';
...
<head>
  ...
  <title>My Bookings • ProLink (Worker)</title>
  ...
  <div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <form method="get" class="bg-white border rounded-xl p-4 mb-6 grid grid-cols-1 md:grid-cols-6 gap-3">
      <div class="md:col-span-2">
        ...
        <input type="text" name="q" value="<?= h($q) ?>" placeholder="Service, location, user..." class="w-full border rounded-lg px-3 py-2">
      </div>
      <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <?php if (!empty($b['notes'])): ?>
                  <div class="mt-2 text-sm text-gray-700"><strong>Notes:</strong> <?= nl2br(h($b['notes'])) ?></div>
                <?php endif; ?>
              </div>
            </div>
